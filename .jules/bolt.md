...
## 2024-05-19 - Scroll Progress Optimization
**Learning:** Synchronous scroll event handlers that read layout properties (like `scrollHeight`) cause layout thrashing and stutter.
**Action:** Always wrap scroll handlers in `window.requestAnimationFrame` with a ticking flag and `{ passive: true }`.
