// ═══════════════════════════════════════════════
// config.js — Barrel re-exporter
//
// All other modules import from here exactly as before.
// Internally, concerns are split into focused files:
//
//   remote-config.js  → AI_KEYS + loadRemoteConfig
//   utils.js          → sanitize, slugify, showToast, etc.
//   constants.js      → CLOUDINARY_CLOUD_NAME, etc.
// ═══════════════════════════════════════════════

// Using Secure Proxy API - All concern modules migrated to Zero-SDK architecture.

// AI keys + remote config loader
export { AI_KEYS, NEWSLETTER_CONFIG, DISPATCH_CONFIG, loadRemoteConfig } from './remote-config.js';

// Utility functions
export {
  cleanEditorHTML,
  sanitize,
  slugify,
  stripTags,
  showToast,
  setBtnLoading,
  parseAIJson,
  validateImageUrl,
  fetchWithTimeout,
  RateLimiter,
}                                    from './utils.js';

// Analytics & UTMs
export {
  injectUtm,
  trackEvent,
}                                    from './analytics.js';

// App constants
export {
  CLOUDINARY_CLOUD_NAME,
  CLOUDINARY_UPLOAD_PRESET,
}                                    from './constants.js';
