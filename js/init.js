...
window.renderPosts = (...args) =>
  typeof renderPosts !== "undefined" ? renderPosts(...args) : null;
window.loadUpstoxMarketData = (...args) =>
  typeof loadUpstoxMarketData !== "undefined"
    ? loadUpstoxMarketData(...args)
    : null;
window.loadPosts = (...args) =>
  typeof loadPosts !== "undefined" ? loadPosts(...args) : null;
window.loadIndiaCalendarData = (...args) =>
  typeof loadIndiaCalendarData !== "undefined"
    ? loadIndiaCalendarData(...args)
    : null;
window.loadForexFactoryData = (...args) =>
  typeof loadForexFactoryData !== "undefined"
    ? loadForexFactoryData(...args)
    : null;
window.initIntelHub = (...args) =>
  typeof initIntelHub !== "undefined" ? initIntelHub(...args) : null;
window.initTVAdvChart = (...args) =>
  typeof initTVAdvChart !== "undefined" ? initTVAdvChart(...args) : null;
window.pollMarkets = (...args) =>
  typeof pollMarkets !== "undefined" ? pollMarkets(...args) : null;
window.handleSearch = (q) => {
  ...
  const filtered = allPosts.filter((p) => {
    const inCat =
      currentCat === "all" ||
      (p.category || "").toLowerCase() === currentCat.toLowerCase();
    const inText =
      !query ||
...
window.filterByCategory = function (cat) {
  ...
  document.querySelectorAll(".filter-chip").forEach((b) => {
    ...
    b.classList.toggle(
      "active",
      chipCat === currentCat || (currentCat === "all" && chipCat === "all"),
    );
  });
  const searchVal =
    document.getElementById("postSearch")?.value?.trim().toLowerCase() || "";
  if (searchVal) {
...
const forexEsc = (v) =>
  String(v || "")
    .replace(/&/g, "&amp;")
    .replace(/</g, "&lt;");
const forexFmtDate = (raw, mode = "local") => {
...
const forexSourceLabel = (src) =>
  src === "forexfactory"
    ? "ForexFactory"
    : src === "tradingeconomics"
      ? "TradingEconomics"
      : "Market Desk";
const forexAnalystView = (event) => {
  const t = String(event?.title || "").toLowerCase();
  if (t.includes("cpi") || t.includes("inflation"))
    return "Analyst view: Inflation surprise risk can reprice rate-cut expectations quickly.";
  if (
    t.includes("employment") ||
    t.includes("non-farm") ||
    t.includes("payroll")
  )
    return "Analyst view: Labor momentum can shift USD yields and index futures direction.";
  if (
    t.includes("pmi") ||
    t.includes("manufacturing") ||
    t.includes("services")
  )
    return "Analyst view: Growth momentum signal; watch cyclical sectors and FX beta pairs.";
  if (
    t.includes("rate") ||
    t.includes("fomc") ||
    t.includes("ecb") ||
    ...
    return "Analyst view: Policy guidance can trigger broad cross-asset volatility in minutes.";
  return "Analyst view: High-impact macro print; keep tighter stops around release minute.";
};
const forexBuildAnalysis = (events) => {
  const byCountry = {};
  ...
  const top = Object.entries(byCountry)
    ...
    .slice(0, 2);
  if (!top.length)
    return "No high-impact events were identified in the current response.";
...
const getForexElements = () => ({
  listEl: document.getElementById("forexCalendarList"),
  chipEl: document.getElementById("forexFeedChip"),
  metaEl: document.getElementById("forexCalendarMeta"),
  analysisEl: document.getElementById("forexCalendarAnalysis"),
  summaryEl: document.getElementById("forexCalendarSummary"),
  snapshotEl: document.getElementById("forexCalendarSnapshot"),
  countryMixChartEl: document.getElementById("forexCountryMixChart"),
  surpriseChartEl: document.getElementById("forexSurpriseChart"),
  sessionGaugeChartEl: document.getElementById("forexSessionGaugeChart"),
  sessionGaugeNoteEl: document.getElementById("forexSessionGaugeNote"),
  deskExplainEl: document.getElementById("forexDeskExplain"),
  pastListEl: document.getElementById("forexCalendarPastList"),
  filterEl: document.getElementById("forexCalendarCountryFilter"),
  titleFilterEl: document.getElementById("forexCalendarTitleFilter"),
  impactEl: document.getElementById("forexCalendarImpactFilter"),
  sortEl: document.getElementById("forexCalendarSort"),
  limitEl: document.getElementById("forexCalendarLimit"),
  tzEl: document.getElementById("forexCalendarTimezone"),
  skeletonEl: document.getElementById("forexCalendarSkeleton"),
});

const applyForexCalendarFilters = () => {
  const els = getForexElements();
  ...
  const q = String(filterEl.value || "")
    .trim()
    .toUpperCase();
  const tq = String(titleFilterEl.value || "")
    .trim()
    .toLowerCase();
  ...
  const nextDate = filtered
    .map((e) => e.date || e.Date || e.datetime)
    .find(Boolean);
  summaryEl.textContent = `Summary: ${filtered.length} matched · Top country: ${topCountry ? `${topCountry[0]} (${topCountry[1]})` : "N/A"} · Next release: ${forexFmtDate(nextDate, tzMode)}`;
  ...
  if (window.google && google.visualization) {
    ...
    const options = {
      ...
      legend: {
        position: "bottom",
        textStyle: { color: "rgba(191,161,0,0.8)", fontSize: 9 },
      },
    };
    new google.visualization.PieChart(
      document.getElementById("forexCountryMixChart"),
    ).draw(data, options);
  }

  const surpriseRows = filtered
    .map((e) => {
      const a = forexParseNum(e.actual);
      const f = forexParseNum(e.forecast);
  ...
  if (window.google && google.visualization && surpriseRows.length) {
    ...
    new google.visualization.BarChart(
      document.getElementById("forexSurpriseChart"),
    ).draw(data, options);
  }
  ...
  const peak = Object.entries(sess).sort((a, b) => b[1] - a[1])[0] || [
    "asia",
    0,
  ];
  const peakMap = {
  ...
  if (window.google && google.visualization) {
    ...
    new google.visualization.Gauge(
      document.getElementById("forexSessionGaugeChart"),
    ).draw(data, options);
  }
      ...
      .map(
        ...
        <li class="calendar-item">
          <div class="calendar-item-head">
            <span class="calendar-country">${forexEsc(e.country || "FX")}</span>
            <span class="calendar-impact">${forexEsc(e.impact || "High")}</span>
          </div>
          <span class="calendar-title">${forexEsc(e.title || "Untitled event")}<span class="calendar-view">${forexEsc(forexAnalystView(e))}</span></span>
          <span class="calendar-date">${forexFmtDate(e.date || e.Date || e.datetime, tzMode)}</span>
          <div class="calendar-points">
            <div class="calendar-point">
              <div class="calendar-point-label">Actual</div>
              <div class="calendar-point-value">${forexEsc(e.actual || "Pending")}</div>
            </div>
            <div class="calendar-point">
              <div class="calendar-point-label">Forecast</div>
              <div class="calendar-point-value">${forexEsc(e.forecast || "--")}</div>
            </div>
            <div class="calendar-point">
              <div class="calendar-point-label">Previous</div>
              <div class="calendar-point-value">${forexEsc(e.previous || "--")}</div>
            </div>
          </div>
        </li>
      `,
  ...
  if (!pastEvents.length) {
    pastListEl.innerHTML =
      '<li class="calendar-empty">No recent past events in current filter set.</li>';
  } else {
    pastListEl.innerHTML = pastEvents
      .map(
        (e) => `
      <li class="calendar-past-item">
        ...
        <span class="calendar-title">${forexEsc(e.title || "Untitled event")}</span>
        <span class="calendar-date">${forexFmtDate(e.date || e.Date || e.datetime, tzMode)}</span>
      </li>
    `,
      )
...
const setForexCalendarState = (
  ...
  metaEl.textContent = `Source: ${forexSourceLabel(source)} · Events: ${events.length || 0} · Updated: ${forexNowStamp()}`;
  if (isError) {
    listEl.innerHTML = `<li class="calendar-empty">Calendar feeds currently unavailable. Please check back shortly.</li>`;
    analysisEl.textContent =
      "Analyst view: Macro board temporarily unavailable; keep risk lower around scheduled sessions.";
    ...
    pastListEl.innerHTML =
      '<li class="calendar-empty">Past events unavailable.</li>';
    return;
  }
  analysisEl.textContent = `Analyst view: ${forexBuildAnalysis(events)}`;
  ...
  forexCalendarRaw = sorted.map(({ __ts, ...rest }) => ({
    ...rest,
    date: rest.date || new Date(__ts).toISOString(),
  }));
  const pastSeed = hydrated
    .slice()
    .sort((a, b) => b.__ts - a.__ts)
    .slice(0, Math.min(6, hydrated.length));
  ...
  applyForexCalendarFilters();
};

async function loadForexFactoryData() {
  const els = getForexElements();
  if (!els.listEl || !els.skeletonEl) return false;

  applyForexFiltersGlobal = applyForexCalendarFilters;

  const candidates = [ENDPOINTS.upstox];
  for (const base of [
    ...new Set(candidates.map((u) => String(u).replace(/\/+$/, ""))),
  ]) {
    try {
      els.skeletonEl.style.display = "grid";
      ...
      setForexCalendarState(null, "Live", events, json.source);
      return true;
    } catch (_) {}
  }
  setForexCalendarState("down", "Down", [], "none", true);
  return false;
}
...
async function loadUpstoxMarketData(opts = {}) {
  ...
  const setAgeLabels = (stamp) => {
    const ageSec = stamp
      ? Math.max(0, Math.round((Date.now() - stamp) / 1000))
      : null;
    const txt = ageSec == null ? "Age: --" : `Age: ${ageSec}s`;
    Object.values(ageEls).forEach((el) => {
  ...
  const fallbackBySegment = {
    indices: [
      { symbol: "Nifty 50", price: 22819.6, change: -0.86 },
      { symbol: "Nifty Bank", price: 52274.6, change: -2.67 },
      { symbol: "Sensex", price: 75124.28, change: -0.61 },
      { symbol: "Nifty Midcap", price: 50874.22, change: 0.33 },
      { symbol: "Nifty IT", price: 36942.9, change: 0.58 },
      { symbol: "Nifty Auto", price: 21984.42, change: -0.22 },
      { symbol: "Nifty Pharma", price: 20114.8, change: 0.47 },
      { symbol: "Nifty PSU Bank", price: 7124.1, change: 1.11 },
    ],
    stocksOptions: [
      { symbol: "RELIANCE", price: 2940.25, change: 0.42 },
      { symbol: "HDFCBANK", price: 1622.4, change: -0.18 },
      { symbol: "ICICIBANK", price: 1114.85, change: 0.44 },
      { symbol: "INFY", price: 1528.35, change: -0.27 },
      { symbol: "TCS", price: 4012.6, change: 0.13 },
      { symbol: "SBIN", price: 768.1, change: 0.76 },
      { symbol: "BHARTIARTL", price: 1279.95, change: -0.06 },
      { symbol: "LT", price: 3721.55, change: 0.22 },
      { symbol: "KOTAKBANK", price: 1762.3, change: 0.31 },
      { symbol: "AXISBANK", price: 1112.4, change: -0.11 },
    ],
    bonds: [
      { symbol: "India 10Y G-Sec", price: 100.15, change: -0.04, yield: 7.12 },
      { symbol: "India 5Y G-Sec", price: 99.74, change: 0.03, yield: 7.03 },
      { symbol: "India 14Y G-Sec", price: 98.88, change: -0.02, yield: 7.2 },
      { symbol: "SDL 2033", price: 97.61, change: 0.01, yield: 7.35 },
      { symbol: "T-Bill 364D", price: 99.22, change: 0.02, yield: 6.88 },
      { symbol: "Corp AAA 5Y", price: 101.07, change: 0.0, yield: 7.54 },
    ],
    currency: [
      {
        symbol: "USDINR",
  ...
  const renderCards = (el, items, segmentKey) => {
    ...
    const detailText2 = (c, key) => {
      ...
      if (key === "bonds") {
        return Number.isFinite(prev)
          ? `Prev ${fmtNum(prev)} · Δ ${Number(c.change || 0).toFixed(2)}%`
          : `Δ ${Number(c.change || 0).toFixed(2)}%`;
      }
      if (Number.isFinite(high) && Number.isFinite(low))
        return `Range ${fmtNum(low)} - ${fmtNum(high)}`;
    ...
    el.innerHTML = items
      .map((raw) => {
        ...
        const codeText =
          symbolText.replace(/[^A-Za-z0-9]/g, "").slice(0, 10) || "--";
        return `
  ...
  for (const base of [
    ...
    try {
      ...
      const qRes = await withTimeout(
        fetch(`${base}/quotes?t=${Date.now()}`, { cache: "no-store" }),
        12000,
        "upstox quotes",
      );
      ...
      if (!qJson || !qJson.data || !Object.keys(qJson.data).length) {
        const gRes = await withTimeout(
          fetch(`${base}/global?t=${Date.now()}`, { cache: "no-store" }),
          12000,
          "upstox global",
        );
      ...
      for (const [instrument, val] of Object.entries(data)) {
        const symbol =
          instrument.split(":")[1] || instrument.split("|")[1] || instrument;
        const keySym = String(symbol || "")
          .trim()
          .toUpperCase();
        ...
        const card = {
          ...
          price: last,
          change: pct,
          open: Number(val?.ohlc?.open),
          high: Number(val?.ohlc?.high ?? val?.high),
          low: Number(val?.ohlc?.low ?? val?.low),
          prev: Number(val?.ohlc?.close ?? val?.prev_close_price),
          volume: Number(val?.volume ?? val?.vol ?? val?.total_volume),
          yield: Number(val?.yield ?? val?.ytm ?? val?.yield_to_maturity),
        };
        const isOption =
          /\b(CE|PE)\b/.test(symU) ||
          symU.includes("OPT") ||
        ...
        const isCrypto =
          ...
          symU.includes("DOGE");
        if (isCrypto) {
          segments.crypto.push(card);
          continue;
        }
        if (k.includes("INDEX") || symbol.toUpperCase().includes("NIFTY"))
          segments.indices.push(card);
        else if (
          k.includes("BSE_DEBT") ||
          k.includes("NSE_DEBT") ||
          k.includes("NSE_BD") ||
          symbol.includes("GSEC") ||
          symbol.includes("BOND")
        )
          segments.bonds.push(card);
        else if (
          k.includes("CDS") ||
          k.startsWith("NSE_CDS") ||
          k.startsWith("BSE_CDS")
        )
          segments.currency.push(card);
        else if (
          k.includes("MCX") ||
          k.startsWith("NSE_COM") ||
          k.startsWith("BSE_COM")
        )
          segments.commodities.push(card);
      ...
      try {
        if (cryptoCache && Date.now() - cryptoCacheAt < CRYPTO_CACHE_TTL) {
          ...
          if (cryptoRows.length) {
            cryptoCache = cryptoRows;
            cryptoCacheAt = Date.now();
            segments.crypto = cryptoRows;
          }
        }
      } catch (_) {}
      ...
      renderCards(
        segmentStocksOptions,
        segments.stocksOptions.slice(0, 30),
        "stocksOptions",
      );
      ...
      renderCards(
        segmentCommodities,
        segments.commodities.slice(0, 16),
        "commodities",
      );
...
async function loadIndiaCalendarData() {
  ...
  const indiaView = (e) => {
    ...
    if (t.includes("cpi") || t.includes("wpi") || t.includes("inflation"))
      return "Analyst view: Inflation trajectory guides RBI reaction function, front-end yields, and rate-sensitive equity sectors. A print above forecast usually hardens rates expectations and can pressure duration-heavy pockets.";
    if (t.includes("rbi") || t.includes("policy") || t.includes("rate"))
      return "Analyst view: RBI tone, vote split, and liquidity guidance can reprice the entire curve quickly. Watch immediate transmission to INR, banks, and high-beta rate plays.";
    if (
      t.includes("industrial") ||
      t.includes("production") ||
      t.includes("gdp")
    )
      return "Analyst view: Growth momentum here affects cyclicals, capex themes, and earnings confidence. Upside surprises tend to support industrials, while misses can revive defensives.";
    if (t.includes("trade") || t.includes("current account"))
      return "Analyst view: External-balance data informs INR stability and imported inflation risk. A wider deficit can increase currency sensitivity and hedging demand.";
  ...
  const buildIndiaDeskMetrics = (events) => {
    ...
    for (const e of events) {
      ...
      if (
        ...
        title.includes("current account") ||
        title.includes("fx reserves")
      )
        extSignal += 1;
    ...
    const holdProb = Math.max(
      35,
      Math.min(80, 60 + inflationSignal * 5 - growthSignal * 3),
    );
    const cutProb = Math.max(
      5,
      Math.min(45, 100 - holdProb - Math.max(10, highCount * 2)),
    );
    const hikeProb = Math.max(5, 100 - holdProb - cutProb);
    const inrPressure =
      extSignal * 0.6 + inflationSignal * 0.4 - growthSignal * 0.2;
    const total = Math.max(events.length, 1);
    const confidence = Math.round(
      ((withForecast / total) * 0.45 +
        (datedEvents / total) * 0.25 +
        (events.filter((e) => String(e.impact || "").trim().length > 0).length /
          total) *
          0.15 +
        (Object.values(sectorHits).filter((v) => v > 0).length / 5) * 0.15) *
        100,
    );
    surprises.sort((a, b) => Math.abs(b.diff) - Math.abs(a.diff));
    const topSurprises = surprises.slice(0, 3);
    const topSector =
      Object.entries(sectorHits).sort((a, b) => b[1] - a[1])[0]?.[0] || "Banks";
    const regimeTag =
      inflationSignal >= growthSignal + 1
        ? "Inflation-Led"
        : growthSignal >= inflationSignal + 1
          ? "Growth-Led"
          : "Balanced";
    const thresholdAlert =
      highCount >= 4
        ? "High alert: cluster of high-impact events this cycle."
        : highCount >= 2
          ? "Moderate alert: keep tighter intraday risk around release windows."
          : "Low alert: normal macro cadence expected.";
    const breadth = surprises.length
      ? `${surprises.filter((s) => s.diff > 0).length} positive vs ${surprises.filter((s) => s.diff < 0).length} negative surprises`
      : "No surprise breadth yet";
    return {
      holdProb,
      cutProb,
      hikeProb,
  ...
  const canonicalIndiaEventKey = (title) => {
    ...
    if (t.includes("industrial production") || t.includes("iip"))
      return "india-iip";
    ...
    if (
      t.includes("rbi") &&
      (t.includes("policy") || t.includes("rate") || t.includes("repo"))
    )
      return "rbi-policy-rate";
  ...
  const buildQoqHistorySeries = (event, allEvents) => {
    ...
    let history = Array.from(byQuarter.entries()).map(([label, row]) => ({
      label,
      value: row.value,
      ts: row.ts,
    }));
    ...
    if (!history.length) {
      ...
      if (Number.isFinite(prev))
        history.push({
          ts: currTs - 91 * 24 * 3600 * 1000,
          value: prev,
          label: quarterLabel(currTs - 91 * 24 * 3600 * 1000),
        });
      if (Number.isFinite(act))
        history.push({ ts: currTs, value: act, label: quarterLabel(currTs) });
      else if (Number.isFinite(prev))
        history.push({ ts: currTs, value: prev, label: quarterLabel(currTs) });
    }
    ...
    if (Number.isFinite(fcst) && history.length) {
      ...
      forecastPoint = {
        label: `${quarterLabel(forecastTs)}F`,
        value: fcst,
        ts: forecastTs,
        forecast: true,
      };
  ...
  const renderQoqGoogleChart = (history, forecastPoint) => {
    ...
    qoqChartEl.innerHTML =
      '<div id="indiaQoqGoogleChart" style="width:100%;height:190px"></div>';
    ...
    const cols = [
      { type: "string", label: "Quarter" },
      { type: "number", label: "Historical" },
      { type: "string", role: "annotation" },
      ...(hasForecast ? [{ type: "number", label: "Forecast" }] : []),
    ];
    ...
    const options = {
      ...window.CHART_THEME,
      chartArea: { ...window.CHART_THEME.chartArea, left: 60 },
      series: {
        0: { color: "#BFA100", areaOpacity: 0.1, lineWidth: 3 },
        ...(hasForecast
          ? { 1: { color: "#FFB800", lineDashStyle: [4, 4], pointSize: 5 } }
          : {}),
      },
      hAxis: { ...window.CHART_THEME.hAxis, title: "Quarterly Period" },
      vAxis: { ...window.CHART_THEME.vAxis, title: "Institutional Delta %" },
    };
  ...
  const renderQoq = (event) => {
    const { history, forecastPoint } = buildQoqHistorySeries(
      event,
      indiaCalendarHistoryRaw,
    );
    if (!history.length) {
      qoqTitleEl.textContent = `India QoQ Historical Trend · ${event.title || "Selected Event"}`;
      qoqChartEl.innerHTML =
        '<div class="calendar-empty">No numeric values available for this event.</div>';
      return;
    }
    qoqTitleEl.textContent = `India QoQ Historical Trend · ${event.title || "Selected Event"}`;
  ...
  const render = () => {
    ...
    const topImpact = events.filter((e) =>
      String(e.impact || "")
        .toLowerCase()
        .includes("high"),
    ).length;
    ...
    const surpriseText = m.topSurprises.length
      ? m.topSurprises
          .map(
            (s) => `${s.title}: ${s.diff > 0 ? "+" : ""}${s.diff.toFixed(2)}`,
          )
          .join(" | ")
      : "No actual-vs-forecast pairs yet";
    const focusEvents =
      events
        .filter((e) => String(e.title || "").trim().length > 0)
        .slice(0, 6)
        .map((e) => esc(e.title))
        .join(" · ") || "No event names available";
    const eventDescriptionFor = (title) => {
      const t = String(title || "").toLowerCase();
      if (t.includes("cpi") || t.includes("inflation"))
        return "Tracks retail inflation pressure and directly affects RBI policy expectations, short-end yields, and rate-sensitive sectors.";
      if (t.includes("wpi"))
        return "Measures producer-level price pressure and helps anticipate margin trends and pass-through risks across manufacturing-heavy sectors.";
      if (
        t.includes("industrial") ||
        t.includes("production") ||
        t.includes("iip")
      )
        return "Captures output momentum and informs growth-sensitive positioning across industrials, metals, and capex-linked names.";
      if (t.includes("trade balance") || t.includes("current account"))
        return "Signals external-balance stress and potential INR volatility via import bill dynamics and dollar demand.";
      if (t.includes("services pmi") || t.includes("manufacturing pmi"))
        return "Shows business activity breadth; sustained expansion supports earnings confidence and cyclical participation.";
      if (
        t.includes("rbi") ||
        t.includes("policy") ||
        t.includes("repo") ||
        ...
        return "Defines policy stance, liquidity conditions, and rate-path signaling that can reprice bonds, banks, and the INR quickly.";
      if (t.includes("gdp"))
        return "Broad growth anchor for earnings expectations, valuation multiples, and medium-term risk appetite.";
    ...
    const keyEventDetails =
      events
        ...
        .map(
          (e) =>
            `<div><b>${esc(e.title)}:</b> ${eventDescriptionFor(e.title)}</div>`,
        )
        .join("") ||
      "<div><b>Key Event Notes:</b> waiting for event descriptions.</div>";
    snapshotEl.innerHTML = `
    ...
    `;
    if (window.google && google.visualization) {
      const probData = google.visualization.arrayToDataTable([
      ...
      new google.visualization.ColumnChart(
        document.getElementById("indiaProbChart"),
      ).draw(probData, {
        ...window.CHART_THEME,
        hAxis: { ...window.CHART_THEME.hAxis, title: "Policy Catalyst" },
        vAxis: { ...window.CHART_THEME.vAxis, title: "Confidence %" },
        legend: { position: "top", alignment: "center" },
      });
      const inrData = google.visualization.arrayToDataTable([
        ["Label", "Value"],
        ["INR Stress", Math.max(0, 50 + m.inrPressure * 10)],
      ]);
      new google.visualization.Gauge(
        document.getElementById("indiaInrGaugeChart"),
      ).draw(inrData, {
        width: 120,
        height: 120,
        redFrom: 70,
      ...
      const sectorData = google.visualization.arrayToDataTable([
        ["Sector", "Sensitivity"],
        ...Object.entries(m.sectorHits).map(([k, v]) => [k, v]),
      ]);
      new google.visualization.BarChart(
        document.getElementById("indiaSectorChart"),
      ).draw(sectorData, {
        ...window.CHART_THEME,
        chartArea: { ...window.CHART_THEME.chartArea, left: 90, width: "70%" },
        hAxis: {
          ...window.CHART_THEME.hAxis,
          title: "Institutional Exposure Score",
        },
        vAxis: {
          ...window.CHART_THEME.vAxis,
          title: "Sector High-Density Vertical",
        },
        legend: { position: "top", alignment: "center" },
      });
    }
    ...
    `,
        ...
        .join("") ||
      '<li class="calendar-empty">No India events available.</li>';
  ...
  for (const base of candidates) {
    try {
      const res = await withTimeout(
        fetch(`${base}/calendar-india`),
        12000,
        "india calendar worker",
      );
      if (!res.ok) continue;
      const json = await res.json();
      ...
      const hydrated = raw.map((e, i) => {
        const t = new Date(e.date || e.Date || e.datetime || 0).getTime();
        const ts =
          Number.isFinite(t) && t > 0 ? t : now + (i + 1) * 6 * 3600 * 1000;
...
function initForexCalendarControls() {
  ...
  if (!refreshEl.dataset.bound) {
    ...
    const refilter = () => {
      if (applyForexFiltersGlobal) applyForexFiltersGlobal();
      else loadForexFactoryData().catch(() => false);
    };
    refreshEl.addEventListener("click", () =>
      loadForexFactoryData().catch(() => false),
    );
...
async function initAuth() {
  try {
    ...
    const user = res.user;
    const isAdmin =
      user.role === "admin" || user.email === "abhishekdutta1996@gmail.com";
    ...
    if (isAdmin) {
      ...
      footerLink.textContent = "Admin Dashboard";
...
async function loadPosts() {
  try {
    ...
    if (postsTitleEl)
      postsTitleEl.textContent = indexUserIsAdmin
        ? "Institutional Strategic Ledger"
        : "Recent Market Pulses (Last 3 Generations)";
    const firestorePosts = allPosts.filter((p) => !p.isAI);
    const totalViews = firestorePosts.reduce(
      (sum, p) => sum + normalizeViews(p.views),
      0,
    );
    ...
    if (aiPulse) {
      ...
      fetch(jsonPath)
        ...
        .catch((e) => {
          console.warn(
            "⚠️ [Init] Policy Hydration failed (Falling back to excerpt):",
            e.message,
          );
...
function renderPosts(posts) {
  ...
  grid.innerHTML = safePosts
    .map((p, i) => {
      ...
      const catClass = isStrategic
        ? "post-category strategic"
        : "post-category";
      const author = esc(
        p.authorName ||
          (p.authorEmail ? String(p.authorEmail).split("@")[0] : "") ||
          "BlogsPro",
      );
      const rawImage = imagesEnabled
        ? DOMPurify.sanitize(String(p.image || "").trim())
        : "";
      const image =
        /^https?:\/\//i.test(rawImage) &&
        rawImage !== "null" &&
        rawImage !== "undefined" &&
        rawImage !== "[object Object]" &&
        !/^javascript:/i.test(rawImage)
          ? rawImage
          : "";
      ...
      if (i === 0) {
        const visual = image
          ? `<div class="featured-visual featured-visual-img"><img src="${image}" alt="${title}" loading="lazy" onerror="this.closest('.featured-visual')?.remove(); this.closest('.post-card')?.classList.add('featured-no-img');" /></div>`
          : "";
...
function renderEmpty() {
  document.getElementById("postsGrid").innerHTML =
    `<div class="empty-state" style="grid-column:1/-1"><div class="empty-state-icon">✍️</div><p>No articles yet. <a href="login.html" style="color:var(--gold)">Publish your first post →</a></p></div>`;
}
...
async function loadAbout() {
  const applyAboutDefaults = () => {
    ...
    document.getElementById("aboutDisplayName2").textContent =
      "Sharp insights for fintech practitioners.";
    document.getElementById("aboutDisplayTagline").textContent =
      "Fintech Insights";
    document.getElementById("aboutDisplayBio").textContent =
      "We publish practical fintech analysis, execution playbooks, and market breakdowns for builders and operators.";
  ...
  try {
    ...
    document.getElementById("aboutDisplayName").textContent =
      data.name || "BlogsPro";
    document.getElementById("aboutDisplayName2").textContent =
      data.heading ||
      data.tagline ||
    ...
    if (data.mission) {
      document.getElementById("aboutDisplayMission").textContent = data.mission;
      document.getElementById("aboutMissionWrap").style.display = "block";
    }
    const socials = document.getElementById("aboutSocials");
    const normalizeSafeUrl = (href) => {
      if (!href) return null;
    ...
    const links = [
      ...
      { key: "twitter", label: "𝕏 Twitter", href: data.twitter || null },
      { key: "linkedin", label: "in LinkedIn", href: data.linkedin || null },
      { key: "website", label: "↗ Website", href: data.website || null },
    ];
    socials.innerHTML = links
      .map((l) => ({ ...l, href: normalizeSafeUrl(l.href) }))
      .filter((l) => l.href)
      .map(
        (l) =>
          `<a class="about-social-link" href="${l.href}" target="_blank" rel="noopener noreferrer">${l.label}</a>`,
      )
      .join("");
...
window.subscribeNewsletter = async (e) => {
  ...
  try {
    ...
    msg.style.cssText =
      "display:block;background:rgba(34,197,94,0.12);border:1px solid rgba(34,197,94,0.3);color:#86efac;margin-top:1.2rem;padding: 1.1rem 1.2rem;border-radius:3px;font-size:0.88rem;font-weight:500";
    msg.textContent = "✓ You're on the list! We'll be in touch.";
  } catch (err) {
    msg.style.cssText =
      "display:block;background:rgba(239,68,68,0.1);border:1px solid rgba(239,68,68,0.3);color:#fca5a5;margin-top:1.2rem;padding: 1.1rem 1.2rem;border-radius:3px;font-size:0.88rem;font-weight:500";
...
function hydratePolicy(latestPost) {
  ...
  const rbiText =
    meta.rbi || (meta.policy && meta.policy.rbi) || latestPost.rbi;
  const sebiText =
    meta.sebi || (meta.policy && meta.policy.sebi) || latestPost.sebi;
  const docs =
    meta.docs || (meta.policy && meta.policy.docs) || latestPost.docs || [];
