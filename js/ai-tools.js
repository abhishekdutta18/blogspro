...
window.aitRunAutoBlog = async () => {
  ...
  for (let i=0; i<count; i++) {
    ...
    try {
      const { addDoc, collection: col, serverTimestamp: sts } = await import("https://www.gstatic.com/firebasejs/10.12.2/firebase-firestore.js");
      const { db: firedb } = await import('./config.js');
      await addDoc(col(firedb,'posts'),{
        title:t, excerpt:meta.summary, content:(await import('./config.js')).sanitize(artR.text),
