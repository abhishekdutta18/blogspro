...
export function workerCandidates(path = "") {
  ...
  // Prefer same-origin API routes first for AI endpoints.
  if (/^api\/(ai|generate-image|validate-url)/.test(p)) {
  ...
  // Respect explicit configuration after same-origin defaults.
