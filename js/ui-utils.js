...
(function () {
  ...
  // We use `requestAnimationFrame` with a ticking flag (`isScrolling`) to throttle
  // scroll events and prevent layout thrashing caused by reading layout properties
  // (`scrollTop`, `scrollHeight`) synchronously on every scroll event.
  // We also use `{ passive: true }` to tell the browser that this event listener
  // will not call `preventDefault()`, improving scrolling smoothness.
