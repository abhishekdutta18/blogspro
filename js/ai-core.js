...
// ── Provider 2: Groq ─────────────────────────
// Groq model priority — ordered by TPM limit (high to low) for parallel safety
...
const GROQ_MODELS = [
  ...
  { id: 'moonshotai/kimi-k2-instruct',          name: 'Kimi K2'        },  // best quality but low TPM
...
// Once a provider fails, skip it for the rest of the session
...
export async function callAI(prompt, silent = false, forceModel = 'auto', maxTokens = 4000) {
  ...
  if (!_providerFailed.groq) {
    ...
    // Fall through to Gemini on ANY Groq error
    markFailed('groq');
    if (!silent) {
