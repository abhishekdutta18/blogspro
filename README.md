...
### AI Writing & Briefing Engine
- **Autonomous Daily Briefings** — Every 24h, the system fetches Forex/WSJ data and generates a full SEO-optimized briefing.
...
- Auto-generated excerpts & professional layout templates (GA4, Sentry)
...
- **Newsletter CRM** — Search, Filter, Export to CSV, and Manual Subscriber management.
- **Automated Newsletter** — "Publish & Notify" trigger with personalized unsubscription links.
- **Sitemap & Ticker** — Self-updating XML sitemap and high-impact Forex ticker.
...
│
├── posts/                   ← Directory for automated AI briefings
│   ├── index.json           ← Static index of all briefings
│   └── ticker.json          ← Live market data for homepage
├── api/
│   └── newsletter-worker.js ← Hardened mailing engine (recursive pagination)
├── scripts/
│   ├── generate-article.js  ← AI Briefing Orchestrator (Parallel Fetch + Gemini)
│   ├── build-static.js      ← SSG build script
│   └── auto-resolve-sentry.js ← Self-healing error resolver
├── firestore.rules          ← Firestore security rules
├── firebase-init.js         ← Legacy Firebase init (root-level)
├── post-view-snippet.js     ← Post view helper
├── sitemap.xml              ← Automated static sitemap
...
| **Autonomous Media Engine (Gemini + Actions)** | Antigravity | **DONE** | `scripts/generate-article.js`, `.github/workflows/daily-post.yml`, `posts/` | 2026-03-26 | End-to-end 08:00 UTC pipeline with parallel data fetching and SEO output. |
| **Newsletter CRM & Scaling** | Antigravity | **DONE** | `api/newsletter-worker.js`, `js/subscribers.js`, `js/newsletter.js` | 2026-03-26 | Hardened recursive mailing engine + Admin Search/Export/Unsubscribe. |
| **Interactive Sentry Bot** | Antigravity | **DONE** | `workers/sentry/index.js`, `.github/workflows/deploy-sentry.yml`, `scripts/auto-resolve-sentry.js` | 2026-03-24 | Added /status, inline Resolve buttons, and auto-issue-deduplication. |
| **Admin UI improvements** | Antigravity | **DONE** | `admin.html`, `js/profile.js`, `index.html` | 2026-03-22 | Profile management and role approval buttons live. |
